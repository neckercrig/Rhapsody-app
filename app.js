...
function loadToday() {
  ...
  document.getElementById('day-label').textContent = entry.dayLabel || '';
  ...
  // Closing — PRAYER or CONFESSION exactly as printed in the PDF
  var closingType = entry.closingType || 'PRAYER';
  document.getElementById('closing-label').textContent = closingType;
  document.getElementById('closing-text').textContent = entry.closing || '';

  // Further Study
  document.getElementById('further-text').textContent = entry.furtherStudy;

  // Bible Reading Plans — both years shown separately as in the PDF
  document.getElementById('plan-1yr').textContent = entry.plan1yr || '';
  document.getElementById('plan-2yr').textContent = entry.plan2yr || '';
...
function openArchiveEntry(dayKey) {
  ...
  var closingType = entry.closingType || 'PRAYER';
  showModal(
    '<p style="font-size:0.7rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.2em;margin-bottom:2px">' +
    d.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }) + '</p>' +
    '<p style="font-size:0.65rem;color:var(--text-faint);margin-bottom:6px">' + (entry.dayLabel || '') + '</p>' +
    '<p class="modal-title">' + entry.title + '</p>' +
    '<div class="scripture-block" style="margin:12px 0">' +
    '<p class="scripture-label">Scripture Reading</p>' +
    '<p class="scripture-text">\u201C' + entry.scripture.text + '\u201D</p>' +
    '<p class="scripture-ref">\u2014 ' + entry.scripture.ref + '</p>' +
    '</div>' +
    '<div style="max-height:32vh;overflow-y:auto;color:var(--text);font-size:0.88rem;line-height:1.75;padding-right:4px">' +
    ...
    '<p style="font-size:0.62rem;color:var(--gold);text-transform:uppercase;letter-spacing:0.15em;margin-bottom:6px">' + closingType + '</p>' +
    '<p style="font-size:0.85rem;line-height:1.65;color:var(--text)">' + (entry.closing || '') + '</p>' +
    '</div>' +
    '<div style="margin-top:10px;padding:10px 12px;background:var(--surface);border-radius:10px;border:1px solid var(--border)">' +
    '<p style="font-size:0.62rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.15em;margin-bottom:4px">Further Study</p>' +
    '<p style="font-size:0.8rem;color:var(--text-dim)">' + (entry.furtherStudy || '') + '</p>' +
    '</div>' +
    '<div style="margin-top:8px;padding:10px 12px;background:var(--surface);border-radius:10px;border:1px solid var(--border)">' +
    '<p style="font-size:0.62rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.15em;margin-bottom:6px">Bible Reading Plans</p>' +
    '<p style="font-size:0.75rem;color:var(--text-dim);margin-bottom:3px"><span style="color:var(--gold)">1-Year:</span> ' + (entry.plan1yr || '') + '</p>' +
    '<p style="font-size:0.75rem;color:var(--text-dim)"><span style="color:var(--gold)">2-Year:</span> ' + (entry.plan2yr || '') + '</p>' +
    '</div>' +
...
function startQuiz(mode, dateKey) {
  ...
  if (mode === 'daily') {
    ...
    const key = dateKey || getTodayKey();
    const entry = getContentForDate(key);
    if (!entry?.discoveryQuiz?.length) { showToast('Discovery Quiz not available yet'); return; }
    questions = entry.discoveryQuiz.map(q => Object.assign({}, q, { type: 'discovery' }));
  } else if (mode === 'rapid') {
...
function renderQuestion() {
  ...
  const isDiscovery = state.quiz.mode === 'discovery';
  state.quiz.answered = false;

  const qNumEl = document.getElementById('quiz-q-num');
  qNumEl.textContent = (isDiscovery ? '🔍 Discovery · ' : '') + 'Question ' + (idx + 1) + ' of ' + total;
  qNumEl.style.color = isDiscovery ? '#64B4FF' : 'var(--text-dim)';
  ...
  optsEl.innerHTML = q.options.map((opt, i) =>
    '<button class="quiz-option' + (isDiscovery ? ' discovery-option' : '') + '" onclick="selectAnswer(' + i + ')">' +
    (isDiscovery ? '' : String.fromCharCode(65 + i) + '. ') + opt + '</button>'
  ).join('');

  document.getElementById('quiz-timer').textContent = '\u23F1 ' + state.quiz.timeLeft;
  document.getElementById('quiz-timer').style.display = isDiscovery ? 'none' : '';
...
function selectAnswer(idx) {
  ...
  const q = state.quiz.questions[state.quiz.current];
  const isDiscovery = q.type === 'discovery';
  // For discovery: "correct" means answered at highest level (index 3)
  const correct = isDiscovery ? (idx === 3) : (idx === q.answer);
  const btns = document.querySelectorAll('.quiz-option');
  btns.forEach(b => b.classList.add('disabled'));

  if (isDiscovery) {
    // Highlight selected only — no wrong/correct colours for discovery
    btns[idx]?.classList.add('correct');
    if (correct) state.quiz.correct++;
    // Show the insight for this question
    const fb = document.getElementById('quiz-feedback');
    fb.className = 'quiz-feedback correct-fb';
    fb.textContent = q.insight || '';
...
function endQuiz() {
  ...
  // Discovery Quiz gets its own result screen
  if (state.quiz.mode === 'discovery') {
    endDiscoveryQuiz();
    return;
  }
...
function exitQuiz() {
  ...
  document.getElementById('quiz-active-view').classList.add('hidden');
  document.getElementById('quiz-result-view').classList.add('hidden');
  document.getElementById('discovery-result-view').classList.add('hidden');
  document.getElementById('quiz-home-view').classList.remove('hidden');
}

function endDiscoveryQuiz() {
  const key = getTodayKey();
  const entry = getContentForDate(key);
  const questions = state.quiz.questions;
  const total = questions.length;

  // Count how many answered at highest level (answer index 3)
  const topAnswers = state.quiz.correct; // we track correct = answered correctly (index 3)
  const pct = Math.round((topAnswers / total) * 100);

  document.getElementById('quiz-active-view').classList.add('hidden');
  const rv = document.getElementById('discovery-result-view');
  rv.classList.remove('hidden');

  // Icon and title based on score
  let icon, title, sub;
  if (pct >= 80) {
    icon = '🌟'; title = 'You\'re Walking in the Word!';
    sub = 'Your answers show a strong alignment with Kingdom thinking. Today\'s article will take you even deeper.';
  } else if (pct >= 60) {
    icon = '📖'; title = 'You\'re on the Right Track';
    sub = 'There\'s real growth happening. Today\'s article has exactly what you need to go further.';
  } else if (pct >= 40) {
    icon = '🔍'; title = 'Great Discoveries Await';
    sub = 'Your honest answers reveal areas where the Word can transform your thinking. Today\'s reading is for you.';
  } else {
    icon = '✨'; title = 'This Article is for You';
    sub = 'Your answers show exactly why today\'s devotional matters. Start reading — it will shift something.';
  }

  document.getElementById('disc-result-icon').textContent = icon;
  document.getElementById('disc-result-title').textContent = title;
  document.getElementById('disc-result-sub').textContent = sub;
  document.getElementById('disc-article-title').textContent = entry ? entry.title : 'Today\'s Devotional';

  // Show one key insight from the questions
  const insightsEl = document.getElementById('disc-insights');
  const randomQ = questions[Math.floor(Math.random() * questions.length)];
  insightsEl.innerHTML = randomQ
    ? '<div class="disc-insight-card"><p class="disc-insight-label">Key Insight</p><p class="disc-insight-text">' + randomQ.insight + '</p></div>'
    : '';
}
...
function shareDiscovery() {
  const key = getTodayKey();
  const entry = getContentForDate(key);
  const title = entry ? entry.title : 'today\'s devotional';
  const text = '🔍 I just took the WordMaster Discovery Quiz on "' + title + '"\n\nTest yourself — see where you really stand on this topic!\n\nThen read the full Rhapsody of Realities article.\n\nJoin: https://t.me/+ybagnIxyxv05MDY0';

  if (tg?.switchInlineQuery) {
    tg.switchInlineQuery(text, ['users']);
  } else if (navigator.share) {
    navigator.share({ title: 'WordMaster Discovery Quiz', text });
  } else {
    // Fallback: copy to clipboard
    if (navigator.clipboard) {
      navigator.clipboard.writeText(text).then(() => showToast('Link copied to clipboard!'));
    } else {
      showToast('Share: t.me/+ybagnIxyxv05MDY0');
    }
  }
}

function openCommunity() {
  const url = 'https://t.me/+ybagnIxyxv05MDY0';
...
function openSubscribe() {
  const url = 'https://rhapsodyofrealities.org';
  if (tg?.openLink) {
    tg.openLink(url);
  } else {
    window.open(url, '_blank');
...
function shareResult() {
  const text = '\uD83C\uDFC6 I scored ' + state.quiz.score + ' pts (' +
    state.quiz.correct + '/' + state.quiz.questions.length + ' correct) in WordMaster Bible Quiz!\n\nCan you beat me? Join: https://t.me/+ybagnIxyxv05MDY0';
  if (tg?.switchInlineQuery) {
    tg.switchInlineQuery(text, ['users']);
  } else if (navigator.share) {
    navigator.share({ title: 'WordMaster Quiz Result', text });
...
function shareChallenge(code, mode) {
  const text = '\u2694 I challenge you to a WordMaster Bible Quiz duel!\nMode: ' + mode + '\nCode: ' + code + '\n\nAccept the challenge on WordMaster!';
  closeModal();
  if (tg?.switchInlineQuery) {
    tg.switchInlineQuery(text, ['users']);
  } else if (navigator.share) {
    navigator.share({ title: 'WordMaster Challenge', text });
  } else if (navigator.clipboard) {
    navigator.clipboard.writeText('Challenge Code: ' + code + ' | Mode: ' + mode).then(() => showToast('Code ' + code + ' copied!'));
  } else {
    showToast('Code: ' + code);
  }
}
...
function shareApp() {
  const text = '\uD83D\uDCD6 WordMaster — Read Rhapsody of Realities daily, take Bible quizzes & challenge friends!\n\nJoin the Lovers of the Word community: https://t.me/+ybagnIxyxv05MDY0';
  if (tg?.shareUrl) {
    tg.shareUrl(window.location.href, text);
  } else if (navigator.share) {
    navigator.share({ title: 'WordMaster', text });
  } else if (navigator.clipboard) {
    navigator.clipboard.writeText(text).then(() => showToast('Link copied!'));
  } else {
    showToast('t.me/+ybagnIxyxv05MDY0');
  }
}
